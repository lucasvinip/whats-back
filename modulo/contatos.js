...
const getUser = function (number) {
  ...
  contatos["whats-users"].forEach(function (date) {
    if (date.number.indexOf(number) >= 0) {
      listJSON.account = date.account
      listJSON.nickname = date.nickname
      listJSON.number = date.number
    }
    else
      return false
  })
...
// console.log(getContact(11987876567));

const getContact = function (id) {
  let contact = contatos["whats-users"][id-1]
